...
func TestEngine_Evaluate_Allow(t *testing.T) {
	...
	for _, tc := range tests {
		got := engine.Evaluate(tc.identity, tc.tool, nil)
...
func TestEngine_Evaluate_Block(t *testing.T) {
	...
	for _, tc := range tests {
		got := engine.Evaluate("any", tc.tool, nil)
		if got.Action != tc.want {
...
func TestEngine_Evaluate_HITL(t *testing.T) {
	...
	for _, tc := range tests {
		got := engine.Evaluate("agent", tc.tool, nil)
...
func TestEngine_Evaluate_FirstMatchWins(t *testing.T) {
	...
	got := engine.Evaluate("agent", "read_database", nil)
...
func TestEngine_Evaluate_IdentityMatch(t *testing.T) {
	...
	got := engine.Evaluate("payment-bot", "execute_payout", nil)
	...
	got = engine.Evaluate("random-agent", "execute_payout", nil)
	if got.Action != ActionBlock {
...
func TestEngine_Evaluate_DefaultDeny(t *testing.T) {
	...
	got := engine.Evaluate("any", "anything", nil)
	if got.Action != ActionBlock {
...
func TestEngine_Evaluate_ParameterMatch(t *testing.T) {
	engine := NewEngine([]config.PolicyCfg{
		{
			Name:   "no-drop",
			Action: ActionBlock,
	...
	tests := []struct {
		...
		{"db_query", map[string]any{"sql": "SELECT * FROM users"}, ActionAllow},
		{"db_query", map[string]any{"sql": "DROP TABLE users"}, ActionBlock},
		{"db_query", nil, ActionAllow}, // no params → skip param policy
	}

	for _, tc := range tests {
		got := engine.Evaluate("agent", tc.tool, tc.params)
		if got.Action != tc.want {
			t.Errorf("Evaluate(%q, %v) = %q; want %q (policy: %s, reason: %s)",
				tc.tool, tc.params, got.Action, tc.want, got.PolicyName, got.Reason)
		}
	}
}

func TestEngine_Evaluate_ParameterMatch_Array(t *testing.T) {
	engine := NewEngine([]config.PolicyCfg{
		{
	...
	tests := []struct {
		...
		{"db_query", map[string]any{"sql": "SELECT * FROM users"}, ActionAllow},
		{"db_query", map[string]any{"sql": "DROP TABLE users"}, ActionBlock},
		{"db_query", map[string]any{"sql": "TRUNCATE users"}, ActionBlock},
		{"db_query", map[string]any{"sql": "ALTER TABLE users ADD COLUMN x INT"}, ActionBlock},
	}
...
func TestEngine_Evaluate_ParameterMatch_MultipleKeys(t *testing.T) {
	engine := NewEngine([]config.PolicyCfg{
		{
			Name:   "block-force-push-to-main",
			Action: ActionBlock,
			Match: config.PolicyMatch{
				Tools:  []string{"git_push"},
				Params: map[string]any{
					"branch": "main",
					"force":  "true",
				},
	...
	tests := []struct {
		...
		{"git_push", map[string]any{"branch": "feature", "force": "false"}, ActionAllow},
		{"git_push", map[string]any{"branch": "main", "force": "false"}, ActionAllow},
		{"git_push", map[string]any{"branch": "main", "force": "true"}, ActionBlock},
		{"git_push", map[string]any{"branch": "main"}, ActionAllow}, // missing "force" → skip
	}
