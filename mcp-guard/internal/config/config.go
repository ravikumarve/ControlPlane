...
type PolicyMatch struct {
	...
	Params   map[string]any    `mapstructure:"params,omitempty"`
}
...
type AlertConfig struct {
	WebhookURL string `mapstructure:"webhook_url"`
	Channel    string `mapstructure:"channel"` // slack, discord, generic
	OnBlock    bool   `mapstructure:"on_block"`
	OnHITL     bool   `mapstructure:"on_hitl"`
	OnRateLimit bool  `mapstructure:"on_rate_limit"`
...
func (c *Config) Validate() []error {
	...
	for i, p := range c.Policies {
		...
		// Validate params patterns
		for paramKey, paramVal := range p.Match.Params {
			switch v := paramVal.(type) {
			case string:
			case []any:
				for j, pv := range v {
					if _, ok := pv.(string); !ok {
						errs = append(errs, fmt.Errorf("policies[%d].match.params[%q][%d] must be a string, got %T", i, paramKey, j, pv))
					}
				}
			default:
				errs = append(errs, fmt.Errorf("policies[%d].match.params[%q] must be a string or array of strings, got %T", i, paramKey, v))
			}
		}

		// Identity wildcard should be explicit
		if p.Match.Identity == "" && len(p.Match.Tools) == 0 && len(p.Match.Params) == 0 {
			errs = append(errs, fmt.Errorf("policies[%d] has no match criteria (empty identity, empty tools, and empty params)", i))
		}
	}
